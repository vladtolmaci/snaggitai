...
BOT_TOKEN      = os.environ.get("BOT_TOKEN",     "8581153858:AAHqr2I5VfKMcb1B4RTAYMkUYHeYh7yFpio")
ANTHROPIC_KEY  = os.environ.get("ANTHROPIC_KEY", "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")
...
GENERATOR_PATH = os.path.join(REPORT_DIR, "generate_v5_newtempl.py")
...
def build_generator_script(report: dict, ai_texts: dict, output_pdf: str) -> str:
    ...
    def to_py(obj):
        """json.dumps but replaces null→None so output is valid Python."""
        return json.dumps(obj, indent=4, ensure_ascii=False).replace(": null", ": None").replace("null,", "None,")

    data_section = f"""
# ═══════════════════════════════════════════════════════════════════════════════
# REPORT DATA — AUTO GENERATED BY SNAGGIT BOT
# ═══════════════════════════════════════════════════════════════════════════════
DATA = {to_py(data_dict)}

TOTALS = {to_py({"critical": sev_counts["critical"], "medium": sev_counts["medium"], "minor": sev_counts["minor"], "total": total, "compliant": sev_counts["compliant"]})}

SUMMARY_OBS = {json.dumps(ai_texts.get("summary_obs",""))}

AREAS = {to_py(areas_list)}

MEP_AREAS = {to_py(mep_list)}

GENERAL_COND = {json.dumps(ai_texts.get("general_condition",""))}

URGENT = {json.dumps(ai_texts.get("urgent","").replace(chr(10), " ").replace(chr(13), " "))}
"""
        ...
        for d in zone.get("defects", []):
            ...
            if fid:
                ...
                if not os.path.exists(local_path):
                    try:
                        tg_file = await msg._bot.get_file(fid)
